...
import pypyr.context.keyvaluepairs
import pytest


def test_comma_string_parses_to_dict():
    """Comma delimited input kvp string should return dictionary."""
    out = pypyr.context.keyvaluepairs.get_parsed_context('key1=value1'
                                                         ',key2=value2'
                                                         ',key3=value3')
...
def test_no_commas_string_parses_to_single_entry():
    ...
    out = pypyr.context.keyvaluepairs.get_parsed_context('key 1=value 2 '
                                                         'value3')
...
def test_no_equals_string_parses_to_single_entry():
    ...
    with pytest.raises(ValueError):
        pypyr.context.keyvaluepairs.get_parsed_context(
            'key1value2,value3')
...
def test_empty_string_throw():
    ...
    with pytest.raises(AssertionError):
        pypyr.context.keyvaluepairs.get_parsed_context(None)
